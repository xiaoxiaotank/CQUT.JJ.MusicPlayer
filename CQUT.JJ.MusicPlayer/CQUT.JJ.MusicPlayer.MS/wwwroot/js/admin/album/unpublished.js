...
document.addEventListener("DOMContentLoaded", function () {
    ...
    $("#exportToExcel").on('click', function () {
        var params = {
            ...
            fileName: '未发布专辑清单',       //用作文件名的字符串。如果缺少，将使用文件名'export.xls'。
            sheetName: 'UnpublishedAlbumList'
        };
...
async function GetRowData() {
    return await
        $.ajax({
            type: "get",
            url: "/Admin/Album/GetUnpublishedAlbums",
            success: function (data) {
                return data;
            }
        })
...
function afterCreateAlbum(data) {
    if (data.isSuccessed) {
        success_prompt(data.message);
        $("#my-modal").modal("hide");
...
function afterUpdateBasicAlbum(data) {
    if (data.isSuccessed) {
        success_prompt(data.message);
        $("#my-modal").modal("hide");
        gridOptions.api.forEachNode(function (node) {
...
function afterDeleteAlbum(data) {
    $("#my-modal").modal("hide");
    if (data.isSuccessed) {
        success_prompt(data.message);
        var sId = 0;
